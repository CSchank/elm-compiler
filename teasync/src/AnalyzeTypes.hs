...
defaultFlags = Flags True False Nothing Nothing Nothing
...
run :: [FilePath] -> Flags -> IO Build.Artifacts
...
runHelp :: FilePath -> [FilePath] -> Reporting.Style -> Flags -> IO (Either Exit.Make Build.Artifacts)
runHelp root paths style (Flags debug optimize maybeOutput _ maybeDocs) =
  ...
  do  desiredMode <- getMode debug optimize
      ...
      case paths of
        ...
        p:ps ->
          buildPaths style root details (NE.List p ps)
