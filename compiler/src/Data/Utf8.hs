...
containsHelp word8# ba# !offset# len# =
  if isTrue# (offset# <# len#) then
    if isTrue# (eqWord8# word8# (indexWord8Array# ba# offset#))
      then True
      else containsHelp word8# ba# (offset# +# 1#) len#
  else
    False
...
chr2 ba# offset# firstWord# =
  let
    !i1# = int8ToInt# (word8ToInt8# firstWord#)
